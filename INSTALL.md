...
The `-j$(nproc)` flag automatically uses all available CPU cores. You can replace it with a fixed number if preferred, e.g. `-j8`.
...
```
cmake -DCMAKE_BUILD_TYPE=Release -DCMAKE_INSTALL_PREFIX=/usr/local -DAPPLICATION_UPDATER=off .
make -j$(nproc)
```
...
On Linux, if you're not building a package and instead intend to install using `make install` it's recommended to set the installation prefix to `/usr/local` instead of `/usr`. This is already included in the standard build command shown in this document.
...
Assuming the recommended installation prefix `/usr/local` has been used (as set in the build command above), this is the directory structure for the installation:

```
/usr/local/bin/ESTACION-PRO
/usr/local/bin/es-pdf-convert
/usr/local/share/applications/org.es_de.frontend.desktop
/usr/local/share/es-de/licenses/*
/usr/local/share/es-de/resources/*
/usr/local/share/es-de/themes/*
/usr/local/share/es-de/LICENSE
...
Note: when building a distro package (DEB, RPM, etc.) use `/usr` as the prefix instead.
