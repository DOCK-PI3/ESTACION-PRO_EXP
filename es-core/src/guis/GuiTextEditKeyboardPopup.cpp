    ...
    , mNavigationRepeatDirY {0}
{
    ...
    const std::shared_ptr<Font> titleFont {Font::get(
        mMenuFontSizeTitle > 0.0f ? mMenuFontSizeTitle :
                                   FONT_SIZE_LARGE * Utils::Localization::sMenuTitleScaleFactor,
        mMenuFontPathTitle.empty() ? Font::getDefaultPath() : mMenuFontPathTitle)};
    const std::shared_ptr<Font> primaryFont {
        Font::get(mMenuFontSizePrimary > 0.0f ? mMenuFontSizePrimary : FONT_SIZE_MEDIUM,
                  mMenuFontPathPrimary.empty() ? Font::getDefaultPath() : mMenuFontPathPrimary)};
    const std::shared_ptr<Font> secondaryFont {
        Font::get(mMenuFontSizeSecondary > 0.0f ? mMenuFontSizeSecondary : FONT_SIZE_SMALL,
                  mMenuFontPathSecondary.empty() ? Font::getDefaultPath() :
                                                   mMenuFontPathSecondary)};

    mTitle = std::make_shared<TextComponent>(title, titleFont, mMenuColorTitle, ALIGN_CENTER);
    ...
    if (mComplexMode) {
        mInfoString = std::make_shared<TextComponent>(infoString, primaryFont, mMenuColorTitle,
                                                      ALIGN_CENTER);
        mGrid.setEntry(mInfoString, glm::ivec2 {0, yPos}, false, true);

        mDefaultValue = std::make_shared<TextComponent>(defaultValue, secondaryFont, mMenuColorTitle,
                                                        ALIGN_CENTER);
