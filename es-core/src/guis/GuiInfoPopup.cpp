    ...
    , mProgress {progress}
{
    ...
    const std::shared_ptr<Font> footerFont {
        Font::get(mMenuFontSizeFooter > 0.0f ? mMenuFontSizeFooter : FONT_SIZE_MINI,
                  mMenuFontPathFooter.empty() ? Font::getDefaultPath() : mMenuFontPathFooter)};

    mText = std::make_shared<TextComponent>("", footerFont, mMenuColorFooter, ALIGN_CENTER);
