...
ComponentList::ComponentList()
    ...
    , mRowHeight {std::round(Font::get(mMenuFontSizePrimary > 0.0f ? mMenuFontSizePrimary :
                                                       FONT_SIZE_MEDIUM,
...
void ComponentList::render(const glm::mat4& parentTrans)
{
    ...
    const float selectorOffsetY {mSelectorBarOffset + std::round(mRowHeight * mMenuSelectorOffsetY)};

    if ((mMenuColorGroupBackground & 0x000000FF) != 0) {
        mRenderer->setMatrix(trans);
        float rowOffsetY {0.0f};
        for (unsigned int i {0}; i < mEntries.size(); ++i) {
            mRenderer->drawRect(0.0f, rowOffsetY, mSize.x, mRowHeight, mMenuColorGroupBackground,
                                mMenuColorGroupBackground, false, mOpacity, mDimming);
            rowOffsetY += mRowHeight;
        }
    }
    ...
    if (mFocused && mOpacity == 1.0f && darkColorScheme) {
        mRenderer->setMatrix(trans);
        mRenderer->drawRect(0.0f, selectorOffsetY, mSize.x, mRowHeight, mMenuColorSelector,
                            mMenuColorSelectorEnd, mMenuColorSelectorGradientHorizontal, mOpacity,
                            mDimming);
    }
    ...
    if (mFocused && !darkColorScheme) {
        if (mOpacity == 1.0f) {
            mRenderer->drawRect(0.0f, selectorOffsetY, mSize.x, mRowHeight, mMenuColorSelector,
                                mMenuColorSelectorEnd, mMenuColorSelectorGradientHorizontal,
                                mOpacity, mDimming,
                                Renderer::BlendFactor::ONE_MINUS_DST_COLOR,
                                Renderer::BlendFactor::ZERO);

            mRenderer->drawRect(0.0f, selectorOffsetY, mSize.x, mRowHeight, 0x777777FF,
