...
{
    // Apply base GuiComponent properties (pos, size, origin, rotation, visible, zIndex, etc.)
    ...
    if (elem->has("cornerSize")) {
        glm::vec2 cs {elem->get<glm::vec2>("cornerSize")};
        // If values look like pixel-scale (>1), use as-is; otherwise scale to screen.
        setCornerSize(cs);
    }

    // Colour: prefer "color"; fall back to "edgeColor" (Batocera terminology).
    if (elem->has("color"))
        setFrameColor(elem->get<unsigned int>("color"));
    else if (elem->has("edgeColor"))
        setFrameColor(elem->get<unsigned int>("edgeColor"));
}
