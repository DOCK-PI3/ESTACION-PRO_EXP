...
TextComponent::TextComponent()
    ...
    , mScrollTime {0.0f}
    , mMaxLength {0.0f}
    , mGlowColor {0x00000000}
    , mGlowSize {0.0f}
    , mGlowOffset {0.0f, 0.0f}
{
}
...
void TextComponent::render(const glm::mat4& parentTrans)
{
    ...
    auto renderFunc = [this](glm::mat4 trans, bool secondPass) {
        ...
        if (mTextCache) {
            ...
            const auto renderTextWithGlow = [this](const glm::mat4& textTrans) {
                const int glowRadius {std::max(0, static_cast<int>(std::round(mGlowSize)))};

                if ((mGlowColor & 0x000000FF) != 0 && glowRadius > 0) {
                    const unsigned int shadowAlpha {static_cast<unsigned int>(glm::clamp(
                        static_cast<int>(std::round((mGlowColor & 0x000000FF) * mColorOpacity)),
                        0, 255))};
                    const unsigned int shadowColor {(mGlowColor & 0xFFFFFF00) | shadowAlpha};

                    mTextCache->setColor(shadowColor);

                    for (int offsetY {-glowRadius}; offsetY <= glowRadius; ++offsetY) {
                        for (int offsetX {-glowRadius}; offsetX <= glowRadius; ++offsetX) {
                            if (offsetX == 0 && offsetY == 0 &&
                                mGlowOffset == glm::vec2 {0.0f, 0.0f}) {
                                continue;
                            }

                            const glm::mat4 glowTrans {
                                glm::translate(textTrans,
                                               glm::vec3 {mGlowOffset.x + offsetX,
                                                          mGlowOffset.y + offsetY, 0.0f})};
                            mRenderer->setMatrix(glowTrans);
                ...
                mRenderer->setMatrix(textTrans);
                mFont->renderTextCache(mTextCache.get());
            };

            renderTextWithGlow(trans);
        }
    };
                               ...
                               unsigned int properties)
{
    ...
    if (properties & COLOR) {
        setGlowColor(elem->has("glowColor") ? elem->get<unsigned int>("glowColor") :
                                             0x00000000);
        setGlowSize(elem->has("glowSize") ? glm::clamp(elem->get<float>("glowSize"), 0.0f, 64.0f) :
                                            0.0f);

        glm::vec2 glowOffset {0.0f, 0.0f};
        if (elem->has("glowOffset")) {
            const glm::vec2 rawOffset {elem->get<glm::vec2>("glowOffset")};
            glowOffset.x = (std::abs(rawOffset.x) <= 1.0f ?
                                rawOffset.x * mRenderer->getScreenWidth() :
                                rawOffset.x);
            glowOffset.y = (std::abs(rawOffset.y) <= 1.0f ?
                                rawOffset.y * mRenderer->getScreenHeight() :
                                rawOffset.y);
        }
        setGlowOffset(glowOffset);
    }
