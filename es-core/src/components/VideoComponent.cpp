    ...
    , mPlayCount {0}
{
    ...
    mConfig.showStaticImageNoVideo = true;
                                ...
                                unsigned int properties)
{
    ...
    if (elem->has("videoCornerRadius"))
        mVideoCornerRadius = glm::clamp(elem->get<float>("videoCornerRadius"), 0.0f, 0.5f) *
                             mRenderer->getScreenWidth();
    else if (elem->has("roundCorners"))
        mVideoCornerRadius = glm::clamp(elem->get<float>("roundCorners"), 0.0f, 0.5f) *
                             mRenderer->getScreenWidth();
    ...
    // Batocera-compatible alias: explicit control over showing snapshots while delay is active.
    mConfig.showStaticImageDelay = false;
    if (elem->has("showSnapshotDelay"))
        mConfig.showStaticImageDelay = elem->get<bool>("showSnapshotDelay");
    else if (mConfig.startDelay != 0)
        mConfig.showStaticImageDelay = true;

    if (elem->has("showSnapshotNoVideo"))
        mConfig.showStaticImageNoVideo = elem->get<bool>("showSnapshotNoVideo");
...
void VideoComponent::startVideoPlayer()
{
    ...
    if (mIsPlaying) {
        // Keep continuity when the same source is already active.
        if (mActiveVideoPath == mVideoPath) {
            mPaused = false;
            mStartTime = 0;
            return;
        }
...
{
    if (!forceRender && !mHasVideo && !mConfig.showStaticImageNoVideo)
        return;
