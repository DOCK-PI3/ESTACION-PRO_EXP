...
{
    const std::shared_ptr<Font> buttonFont {
    ...
    if (mFlatStyle) {
        mButtonText =
            std::make_unique<TextComponent>("", buttonFont, 0xFFFFFFFF, ALIGN_CENTER);
    }
    else {
        mButtonText = std::make_unique<TextComponent>("DELETE", buttonFont, 0xFFFFFFFF,
                                                      ALIGN_CENTER);
...
void ButtonComponent::updateImage()
{
    if (!mEnabled || !mPressedFunc) {
        mBox.setImagePath(mMenuImagePathButtonFilled);
        mBox.setFrameColor(0x770000FF);
        return;
    }

    mBox.setFrameColor(mMenuColorButtonFocused);
    mBox.setImagePath(mFocused ? mMenuImagePathButtonFilled : mMenuImagePathButton);
}
