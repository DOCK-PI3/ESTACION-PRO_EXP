...
HelpComponent::HelpComponent(std::shared_ptr<Font> font)
    ...
    , mFont {font ? font :
                    Font::get(mMenuFontSizeFooter > 0.0f ?
                                  mMenuFontSizeFooter :
                                  (mMenuFontSizeSecondary > 0.0f ?
                                       mMenuFontSizeSecondary :
                                       (Renderer::getIsVerticalOrientation() ?
                                            0.025f * Renderer::getScreenWidth() :
                                            FONT_SIZE_SMALL)),
                              !mMenuFontPathFooter.empty() ? mMenuFontPathFooter :
                              (!mMenuFontPathSecondary.empty() ? mMenuFontPathSecondary :
                                                                 Font::getDefaultPath()))}
    , mFontDimmed {mFont}
    ...
    , mTextColor {mMenuColorFooter}
    , mTextColorDimmed {mMenuColorFooter}
    , mIconColor {mMenuColorSecondary}
    , mIconColorDimmed {mMenuColorSecondary}
                               ...
                               unsigned int properties)
{
    ...
    // Batocera compatibility aliases used by many themes.
    if (elem->has("iconUpDown"))
        mCustomButtons.dpad_updown = elem->get<std::string>("iconUpDown");
    if (elem->has("iconLeftRight"))
        mCustomButtons.dpad_leftright = elem->get<std::string>("iconLeftRight");
    if (elem->has("iconUpDownLeftRight"))
        mCustomButtons.dpad_all = elem->get<std::string>("iconUpDownLeftRight");
    if (elem->has("iconL"))
        mCustomButtons.button_l = elem->get<std::string>("iconL");
    if (elem->has("iconR"))
        mCustomButtons.button_r = elem->get<std::string>("iconR");

    if (elem->has("iconA")) {
        const std::string iconAPath {elem->get<std::string>("iconA")};
        mCustomButtons.button_a_SNES = iconAPath;
        mCustomButtons.button_a_switch = iconAPath;
        mCustomButtons.button_a_PS = iconAPath;
        mCustomButtons.button_a_XBOX = iconAPath;
    }

    if (elem->has("iconB")) {
        const std::string iconBPath {elem->get<std::string>("iconB")};
        mCustomButtons.button_b_SNES = iconBPath;
        mCustomButtons.button_b_switch = iconBPath;
        mCustomButtons.button_b_PS = iconBPath;
        mCustomButtons.button_b_XBOX = iconBPath;
    }

    if (elem->has("iconX")) {
        const std::string iconXPath {elem->get<std::string>("iconX")};
        mCustomButtons.button_x_SNES = iconXPath;
        mCustomButtons.button_x_switch = iconXPath;
        mCustomButtons.button_x_PS = iconXPath;
        mCustomButtons.button_x_XBOX = iconXPath;
    }

    if (elem->has("iconY")) {
        const std::string iconYPath {elem->get<std::string>("iconY")};
        mCustomButtons.button_y_SNES = iconYPath;
        mCustomButtons.button_y_switch = iconYPath;
        mCustomButtons.button_y_PS = iconYPath;
        mCustomButtons.button_y_XBOX = iconYPath;
    }

    if (elem->has("iconStart")) {
        const std::string iconStartPath {elem->get<std::string>("iconStart")};
        mCustomButtons.button_start_SNES = iconStartPath;
        mCustomButtons.button_start_switch = iconStartPath;
        mCustomButtons.button_start_PS123 = iconStartPath;
        mCustomButtons.button_start_PS4 = iconStartPath;
        mCustomButtons.button_start_PS5 = iconStartPath;
        mCustomButtons.button_start_XBOX = iconStartPath;
        mCustomButtons.button_start_XBOX360 = iconStartPath;
    }

    if (elem->has("iconSelect")) {
        const std::string iconSelectPath {elem->get<std::string>("iconSelect")};
        mCustomButtons.button_back_SNES = iconSelectPath;
        mCustomButtons.button_back_switch = iconSelectPath;
        mCustomButtons.button_back_PS123 = iconSelectPath;
        mCustomButtons.button_back_PS4 = iconSelectPath;
        mCustomButtons.button_back_PS5 = iconSelectPath;
        mCustomButtons.button_back_XBOX = iconSelectPath;
        mCustomButtons.button_back_XBOX360 = iconSelectPath;
    }
