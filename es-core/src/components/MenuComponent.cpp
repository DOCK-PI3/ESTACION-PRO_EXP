...
namespace {
std::shared_ptr<Font> getMenuFont(float sizeOverride,
                                  const std::string& pathOverride,
                                  float fallbackSize)
{
    return Font::get(sizeOverride > 0.0f ? sizeOverride : fallbackSize,
                     pathOverride.empty() ? Font::getDefaultPath() : pathOverride);
}
}
    ...
    , mNeedsSaving {false}
{
    ...
    setTitle(title, titleFont ? titleFont :
                             getMenuFont(mMenuFontSizeTitle, mMenuFontPathTitle,
                                         FONT_SIZE_LARGE *
                                             Utils::Localization::sMenuTitleScaleFactor));
