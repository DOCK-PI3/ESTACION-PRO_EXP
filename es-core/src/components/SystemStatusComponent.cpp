...
void SystemStatusComponent::updateGrid()
{
    ...
    if (!mCompatForceBatteryOnly && mHasBluetooth &&
    ...
    if (!mCompatForceBatteryOnly && mHasWifi &&
        Settings::getInstance()->getBool("SystemStatusWifi") &&
        std::find(mEntries.cbegin(), mEntries.cend(), "wifi") != mEntries.cend())
        mDisplayEntries.emplace_back("wifi");
    if (!mCompatForceBatteryOnly && mHasCellular &&
    ...
    const bool showBattery {
        mCompatForceBatteryOnly || Settings::getInstance()->getBool("SystemStatusBattery")};
    if (mHasBattery && showBattery &&
    ...
    mBatteryText = !mCompatForceBatteryOnly && Settings::getInstance()->getBool("SystemStatusBattery") &&
                   Settings::getInstance()->getBool("SystemStatusBatteryPercentage");
                                       ...
                                       unsigned int properties)
{
    ...
    std::string resolvedThemeType {"systemstatus"};

    if (elem == nullptr) {
        elem = theme->getElement(view, element, "controllerActivity");
        if (elem != nullptr)
            resolvedThemeType = "controllerActivity";
    }

    if (elem == nullptr) {
        elem = theme->getElement(view, element, "batteryIndicator");
        if (elem != nullptr)
            resolvedThemeType = "batteryIndicator";
    }

    mCompatForceBatteryOnly = (resolvedThemeType == "batteryIndicator");
    mEntries = sAllowedEntries;
    if (mCompatForceBatteryOnly)
        mEntries = {"battery"};
    ...
    else if (elem->has("activityColor"))
        mColorShift = elem->get<unsigned int>("activityColor");
    else if (elem->has("hotkeyColor"))
        mColorShift = elem->get<unsigned int>("hotkeyColor");
    ...
    if (elem->has("entries")) {
        ...
        // Normalize common Batocera aliases used by third-party themes.
        for (auto& entry : entries) {
            entry = Utils::String::trim(entry);
            if (entry == "bt")
                entry = "bluetooth";
            else if (entry == "network" || entry == "networkicon" || entry == "wifiicon")
                entry = "wifi";
            else if (entry == "planemode" || entry == "planemodeicon" || entry == "airplane" ||
                     entry == "airplanemode")
                entry = "cellular";
            else if (entry == "batteryindicator" || entry == "power")
                entry = "battery";
        }
    ...
    else if (elem->has("itemSpacing")) {
        mEntrySpacing = std::round(glm::clamp(elem->get<float>("itemSpacing"), 0.0f, 0.04f) *
                                   mRenderer->getScreenWidth());
    }
    ...
    // Batocera compatibility aliases commonly used by batteryIndicator/controllerActivity.
    if (elem->has("networkIcon"))
        mIconPathMap["wifi"] = elem->get<std::string>("networkIcon");
    if (elem->has("planemodeIcon"))
        mIconPathMap["cellular"] = elem->get<std::string>("planemodeIcon");
    if (elem->has("incharge"))
        mIconPathMap["battery_charging"] = elem->get<std::string>("incharge");
    if (elem->has("at25"))
        mIconPathMap["battery_low"] = elem->get<std::string>("at25");
    if (elem->has("at50"))
        mIconPathMap["battery_medium"] = elem->get<std::string>("at50");
    if (elem->has("at75"))
        mIconPathMap["battery_high"] = elem->get<std::string>("at75");
    if (elem->has("full"))
        mIconPathMap["battery_full"] = elem->get<std::string>("full");
    if (elem->has("empty"))
        mIconPathMap["battery_low"] = elem->get<std::string>("empty");

    // If only a single generic image path is provided, reuse it for all battery states.
    if (elem->has("imagePath")) {
        const std::string genericPath {elem->get<std::string>("imagePath")};
        if (!genericPath.empty()) {
            mIconPathMap["battery_charging"] = genericPath;
            mIconPathMap["battery_low"] = genericPath;
            mIconPathMap["battery_medium"] = genericPath;
            mIconPathMap["battery_high"] = genericPath;
            mIconPathMap["battery_full"] = genericPath;
        }
    }
}
