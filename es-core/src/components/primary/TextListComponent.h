...
private:
    void onShowPrimary() override
    {
        if (mCursor >= 0 && mCursor < static_cast<int>(mEntries.size())) {
            auto comp = mEntries.at(mCursor).data.entryName;
            if (comp) {
                comp->resetComponent();
    ...
    {
        if (mScrollSound) {
            if (!isScrolling() || !mScrollSound->isPlaying())
                mScrollSound->play();
            return;
        }
                                      ...
                                      unsigned int properties)
{
    ...
    if (elem->has("scrollSound"))
        mScrollSound = Sound::get(elem->get<std::string>("scrollSound"));
}
...
template <typename T> void TextListComponent<T>::onCursorChanged(const CursorState& state)
{
    if (mLastCursor >= 0 && mLastCursor < static_cast<int>(mEntries.size())) {
        auto comp = mEntries.at(mLastCursor).data.entryName;
        if (comp) {
