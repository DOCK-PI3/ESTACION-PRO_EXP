...
private:
    void onShowPrimary() override
    {
        ...
        for (int i {0}; i < static_cast<int>(mEntries.size()); ++i) {
            ...
            auto comp = mEntries.at(i).data.item;
            if (comp && comp->selectStoryboard("scroll")) {
                comp->startStoryboard();
                comp->update(0);
            }
        }
...
void CarouselComponent<T>::addEntry(Entry& entry, const std::shared_ptr<ThemeData>& theme)
{
    ...
    if (!entry.data.item) {
        ...
        if (mTextGlowSize > 0.0f) {
            text->setGlowColor(mTextGlowColor);
            text->setGlowSize(mTextGlowSize);
        }
    ...
    // Wheel carousels position items using dedicated wheel alignment properties.
    const Alignment horizontalAlignment {
        mType == CarouselType::VERTICAL_WHEEL ? mWheelHorizontalAlignment :
                                                mItemHorizontalAlignment};
    const Alignment verticalAlignment {mType == CarouselType::HORIZONTAL_WHEEL ?
                                           mWheelVerticalAlignment :
                                           mItemVerticalAlignment};
...
template <typename T> void CarouselComponent<T>::onDemandTextureLoad()
{
    if constexpr (std::is_same_v<T, FileData*>) {
        ...
        for (int i {center - itemInclusion - itemInclusionBefore};
            ...
            // 1. Siempre priorizar marquee del juego si existe
            ...
            if (!marqueePath.empty() && ResourceManager::getInstance().fileExists(marqueePath)) {
                entry.data.imagePath = marqueePath;
            } else {
                // 2. Si no hay marquee, intentar logo del theme
                std::string logoPath;
                if (theme) {
                    const ThemeData::ThemeElement* logoElem = theme->getElement("gamelist", "logo", "image");
                    if (logoElem && logoElem->has("path")) {
                        logoPath = logoElem->get<std::string>("path");
                        if (!logoPath.empty() && ResourceManager::getInstance().fileExists(logoPath)) {
                            entry.data.imagePath = logoPath;
                        }
                    }
                }
                // 3. Si tampoco hay logo, usar el resto de tipos de imagen
                if (entry.data.imagePath == "") {
                ...
                // 4. Si nada funcionó, usar imagen por defecto
                if (entry.data.imagePath == "")
                                      ...
                                      unsigned int properties)
{
    ...
    // Batocera compatibility aliases for carousel logo-centric properties.
    if (!elem->has("itemSize") && elem->has("logoSize")) {
        const glm::vec2 logoSize {glm::clamp(elem->get<glm::vec2>("logoSize"), 0.05f, 1.0f)};
        mItemSize =
            logoSize * glm::vec2(Renderer::getScreenWidth(), Renderer::getScreenHeight());
    }

    if (!elem->has("maxItemCount") && elem->has("maxLogoCount")) {
        mMaxItemCount = glm::clamp(elem->get<float>("maxLogoCount"), 0.5f, 30.0f);
    }

    if (!elem->has("itemScale") && elem->has("logoScale"))
        mItemScale = glm::clamp(elem->get<float>("logoScale"), 0.2f, 3.0f);

    if (!elem->has("itemRotation") && elem->has("logoRotation"))
        mItemRotation = elem->get<float>("logoRotation");

    if (!elem->has("itemRotationOrigin") && elem->has("logoRotationOrigin"))
    ...
    if (elem->has("logoAlignment")) {
        ...
        if (mType == CarouselType::VERTICAL_WHEEL && !elem->has("wheelHorizontalAlignment")) {
            if (alignment == "left")
                ...
                mWheelHorizontalAlignment = ALIGN_CENTER;
                 ...
                 !elem->has("wheelVerticalAlignment")) {
            if (alignment == "top")
        ...
        else if (!elem->has("itemHorizontalAlignment")) {
            if (alignment == "left")
    ...
    if (mGamelistView && properties && (elem->has("imageType") || elem->has("imageSource"))) {
        ...
        if (mImageTypes.empty()) {
            LOG(LogWarning)
                << "CarouselComponent: Invalid theme configuration, property \""
                << propertyName << "\" "
                   "for element \""
    ...
    // Batocera compatibility: read gamecarouselLogoText companion element (and fallback to
    // logoText) so the game-name fallback text shown in gamelist carousel items inherits the
    // font/color/glow defined in the theme rather than the opaque default black.
    if (mGamelistView) {
        const ThemeData::ThemeElement* logoTextElem {
            theme->getElement(view, "gamecarouselLogoText", "text")};
        if (!logoTextElem)
            logoTextElem = theme->getElement(view, "logoText", "text");
        if (logoTextElem) {
            if (logoTextElem->has("color"))
                mTextColor = logoTextElem->get<unsigned int>("color");
            if (logoTextElem->has("glowColor"))
                mTextGlowColor = logoTextElem->get<unsigned int>("glowColor");
            if (logoTextElem->has("glowSize"))
                mTextGlowSize = glm::clamp(logoTextElem->get<float>("glowSize"), 0.0f, 20.0f);
            if (logoTextElem->has("fontPath") || logoTextElem->has("fontSize")) {
                mFont = Font::getFromTheme(logoTextElem, properties, mFont, 0.0f,
...
{
    const int oldCursor {mLastCursor};

    bool oldCursorHasStoryboard {false};
    if (oldCursor >= 0 && oldCursor < static_cast<int>(mEntries.size())) {
        auto comp = mEntries.at(oldCursor).data.item;
        if (comp) {
    ...
    if (mCursor >= 0 && mCursor < static_cast<int>(mEntries.size())) {
        ...
        if (comp) {
            ...
            if (comp->selectStoryboard("activate")) {
                comp->startStoryboard();
                comp->update(0);
                cursorHasStoryboard = true;
            }
            else if (comp->selectStoryboard("open")) {
                comp->startStoryboard();
                comp->update(0);
                cursorHasStoryboard = true;
            }
