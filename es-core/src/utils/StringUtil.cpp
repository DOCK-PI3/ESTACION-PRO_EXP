...
namespace Utils
{
    namespace String
    {
        namespace
        {
            std::string sanitizePrintfFormatString(const std::string& format)
            {
                // glibc FORTIFY aborts if %n appears in a writable format string.
                // Escape any %...n conversion so it is treated as literal text.
                std::string sanitized;
                sanitized.reserve(format.size());

                for (size_t i {0}; i < format.size(); ++i) {
                    if (format[i] != '%') {
                        sanitized.push_back(format[i]);
                        continue;
                    }
                    ...
                    size_t j {i + 1};
                    while (j < format.size()) {
                        if (std::strchr("diuoxXfFeEgGaAcspn", format[j]) != nullptr)
                            break;
                        ++j;
                    }

                    if (j < format.size() && format[j] == 'n')
                        sanitized += "%%";
                    else
                        sanitized.push_back('%');

                    if (j < format.size()) {
                        sanitized.append(format, i + 1, j - i);
                        i = j;
                    }
                    else {
                        sanitized.append(format, i + 1, format.size() - (i + 1));
                        break;
                    }
                }
        ...
        std::string format(const std::string stringArg, ...)
        {
            ...
            const std::string sanitizedFormat {sanitizePrintfFormatString(stringArg)};
            ...
            const int length {vsnprintf(nullptr, 0, sanitizedFormat.c_str(), copy)};
            va_end(copy);
            ...
            va_copy(copy, args);
            vsnprintf(&buffer[0], length + 1, sanitizedFormat.c_str(), copy);
