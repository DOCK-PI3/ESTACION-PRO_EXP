...
std::vector<SDL_JoystickID> InputManager::getConnectedControllerIDs() const
{
    std::vector<SDL_JoystickID> ids;
    ids.reserve(mControllers.size());

    for (const auto& controller : mControllers)
        ids.emplace_back(controller.first);

    return ids;
}

SDL_JoystickPowerLevel InputManager::getControllerPowerLevel(SDL_JoystickID deviceId) const
{
    const auto it {mJoysticks.find(deviceId)};
    if (it == mJoysticks.cend() || it->second == nullptr)
        return SDL_JOYSTICK_POWER_UNKNOWN;

    return SDL_JoystickCurrentPowerLevel(it->second);
}
