...
namespace
{
unsigned int getSplashBackgroundColor()
{
    constexpr unsigned int defaultSplashBackgroundColor {0x000000FF};
    ...
    try {
        ...
        if (themes.empty())
            return defaultSplashBackgroundColor;

        auto selectedTheme {themes.find(Settings::getInstance()->getString("Theme"))};
        if (selectedTheme == themes.cend())
            selectedTheme = themes.cbegin();

        const std::string themeRootFile {selectedTheme->second.path + "/theme.xml"};
        if (!Utils::FileSystem::exists(themeRootFile))
            return defaultSplashBackgroundColor;

        ThemeData theme;
        std::map<std::string, std::string> emptyVariables;
        theme.loadFile(emptyVariables, themeRootFile, ThemeTriggers::TriggerType::NONE, false);

        const std::vector<std::string> candidateViews {"all", "system", "basic", "detailed",
                                                       "grid", "video", "menu"};
        for (const auto& view : candidateViews) {
            const ThemeData::ThemeElement* splashElement {
                theme.getElement(view, "splash", "splash")};
            if (splashElement != nullptr && splashElement->has("backgroundColor"))
                return splashElement->get<unsigned int>("backgroundColor");
        }
    }
    catch (const std::exception& e) {
        LOG(LogWarning) << "Window::getSplashBackgroundColor(): " << e.what();
    }
...
Window::Window() noexcept
    ...
    , mSplashBackgroundColor {0x000000FF}
...
bool Window::init(bool resized)
{
    ...
    mSplashBackgroundColor = getSplashBackgroundColor();
...
void Window::renderSplashScreen(SplashScreenState state, float progress)
{
    ...
    mRenderer->drawRect(0.0f, 0.0f, mRenderer->getScreenWidth(), mRenderer->getScreenHeight(),
                        mSplashBackgroundColor, mSplashBackgroundColor);
