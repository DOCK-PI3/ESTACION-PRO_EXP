    ...
    , mCustomCollectionSystem {nullptr}
{
    ...
    if (file->getType() == FOLDER) {
        ...
        if (!mFromPlaceholder && file->hasCheevos()) {
            row.elements.clear();
            row.addElement(std::make_shared<TextComponent>(_("VIEW THIS GAME'S ACHIEVEMENTS"),
                                                           Font::get(FONT_SIZE_MEDIUM),
                                                           mMenuColorPrimary),
                           true);
            row.addElement(mMenu.makeArrow(), false);
            row.makeAcceptInputHandler([this, file] {
                mWindow->pushGui(new GuiGameAchievements(file->getSourceFileData()));
            });
            mMenu.addRow(row);
        }
    }
