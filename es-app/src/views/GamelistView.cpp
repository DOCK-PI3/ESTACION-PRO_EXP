...
namespace {
std::string boolToBindingString(const bool value)
{
    return value ? "true" : "false";
}

std::string buildStarsFromRating(const std::string& rating)
{
    float ratingValue {0.0f};
    try {
        ratingValue = std::stof(rating);
    }
    catch (...) {
        return "";
    }

    ratingValue = std::clamp(ratingValue, 0.0f, 1.0f);
    const int stars {static_cast<int>(std::round(ratingValue * 5.0f))};
    return std::string(stars, '*');
}

void updateGameBindingVariables(FileData* file)
{
    if (file == nullptr)
        return;

    SystemData* gameSystem {file->getSourceSystem()};
    if (gameSystem == nullptr)
        gameSystem = file->getSystem();

    if (gameSystem == nullptr || gameSystem->getTheme() == nullptr)
        return;

    const std::string gamePath {file->getPath()};
    const std::string gameName {file->getName()};
    const std::string rating {file->metadata.get("rating")};
    const std::string players {file->metadata.get("players")};

    std::string nameShort {gameName};
    std::string nameExtra;
    const size_t parenPos {gameName.find('(')};
    if (parenPos != std::string::npos) {
        nameShort = Utils::String::trim(gameName.substr(0, parenPos));
        nameExtra = Utils::String::trim(gameName.substr(parenPos));
    }

    std::map<std::string, std::string> vars {
        {"game.name", gameName},
        {"game.rom", Utils::FileSystem::getFileName(gamePath)},
        {"game.stem", Utils::FileSystem::getStem(gamePath)},
        {"game.path", gamePath},
        {"game.favorite", boolToBindingString(file->getFavorite())},
        {"game.hidden", boolToBindingString(file->getHidden())},
        {"game.kidGame", boolToBindingString(file->getKidgame())},
        {"game.folder", boolToBindingString(file->getType() == FOLDER)},
        {"game.placeholder", boolToBindingString(file->isPlaceHolder())},
        {"game.playerCount", players},
        {"game.hasManual", boolToBindingString(!file->getManualPath().empty())},
        {"game.cheevos", boolToBindingString(file->hasCheevos())},
        {"game.hasCheevos", boolToBindingString(file->hasCheevos())},
        {"game.cheevosId", std::to_string(file->getCheevosGameId())},
        {"game.cheevosHash", file->getCheevosHash()},
        {"game.hasSaveState", "false"},
        {"game.hasKeyboardMapping", "false"},
        {"game.gametime", file->metadata.get("playtime")},
        {"game.desc", file->metadata.get("desc")},
        {"game.image", file->getImagePath()},
        {"game.thumbnail", file->getScreenshotPath()},
        {"game.video", file->getVideoPath()},
        {"game.marquee", file->getMarqueePath()},
        {"game.logo", file->getMarqueePath()},
        {"game.fanart", file->getFanArtPath()},
        {"game.titleshot", file->getTitleScreenPath()},
        {"game.manual", file->getManualPath()},
        {"game.boxart", file->getCoverPath()},
        {"game.boxback", file->getBackCoverPath()},
        {"game.wheel", file->getMarqueePath()},
        {"game.mix", file->getMiximagePath()},
        {"game.rating", rating},
        {"game.releasedate", file->metadata.get("releasedate")},
        {"game.releaseyear", file->metadata.get("releasedate").substr(0, 4)},
        {"game.developer", file->metadata.get("developer")},
        {"game.publisher", file->metadata.get("publisher")},
        {"game.genre", file->metadata.get("genre")},
        {"game.players", players},
        {"game.playcount", file->metadata.get("playcount")},
        {"game.lastplayed", file->metadata.get("lastplayed")},
        {"game.lang", ""},
        {"game.region", ""},
        {"game.directory", Utils::FileSystem::getParent(gamePath)},
        {"game.type", file->isPlaceHolder() ? "placeholder" :
                         (file->getType() == FOLDER ? "folder" : "game")},
        {"game.stars", buildStarsFromRating(rating)},
        {"game.nameShort", nameShort},
        {"game.nameExtra", nameExtra},
        {"game.system.name", gameSystem->getName()},
        {"game.system.theme", gameSystem->getThemeFolder()},
        {"game.system.fullName", gameSystem->getFullName()}
    };

    gameSystem->getTheme()->setRuntimeVariables(vars);
}

void triggerGamelistStoryboardRecursive(GuiComponent* component, bool activate)
{
    ...
    for (unsigned int i {0}; i < component->getChildCount(); ++i)
        triggerGamelistStoryboardRecursive(component->getChild(i), activate);
}
} // namespace
...
void GamelistView::onShow()
{
    ...
    for (auto& controllerActivity : mControllerActivityComponents)
        controllerActivity->update(0);
...
void GamelistView::onHide()
{
    ...
    for (unsigned int i {0}; i < getChildCount(); ++i) {
        ...
        const bool hasStoryboard =
            child->storyBoardExists("activate") || child->storyBoardExists("deactivate") ||
            child->storyBoardExists("show") || child->storyBoardExists("close") ||
            child->storyBoardExists("hide") ||
            child->storyBoardExists("");
        if (!hasStoryboard)
            continue;

        triggerGamelistStoryboardRecursive(child, false);
    }
...
void GamelistView::onThemeChanged(const std::shared_ptr<ThemeData>& theme)
{
    ...
    auto attachToParentStack = [&](GuiComponent* component, const std::string& logicalName) {
        const size_t splitPos {logicalName.rfind('.')};
        if (splitPos == std::string::npos)
            return false;

        const std::string parentName {logicalName.substr(0, splitPos)};
        auto it = stackPanels.find(parentName);
        if (it == stackPanels.end())
            return false;

        it->second->addChild(component);
        return true;
    };
    auto addThemedChild = [&](GuiComponent* component, const std::string& elementKey) {
        if (!attachToParentStack(component, getLogicalName(elementKey)))
            addChild(component);
    };

    if (mTheme->hasView("gamelist")) {
        // Limpiar instancias previas para permitir cambiar entre customViews
        mTextList.reset();
        mGrid.reset();
        mCarousel.reset();
        mPrimary = nullptr;
        for (auto& element : mTheme->getViewElements("gamelist").elements) {
            ...
            if (element.second.type == "textlist") {
                ...
                addThemedChild(mPrimary, element.first);
            }
            ...
            if (element.second.type == "stackpanel" &&
                !(element.second.has("visible") && !element.second.get<bool>("visible"))) {
                mStackPanelComponents.push_back(std::make_unique<StackPanelComponent>());
                mStackPanelComponents.back()->setDefaultZIndex(35.0f);
                mStackPanelComponents.back()->applyTheme(theme, "gamelist", element.first, ALL);

                const std::string logicalName {getLogicalName(element.first)};
                stackPanels[logicalName] = mStackPanelComponents.back().get();

                addThemedChild(mStackPanelComponents.back().get(), element.first);
            }
            else if (element.second.type == "container" &&
                     !(element.second.has("visible") && !element.second.get<bool>("visible"))) {
                mGenericContainerComponents.push_back(std::make_unique<ContainerComponent>());
                mGenericContainerComponents.back()->setDefaultZIndex(35.0f);
                mGenericContainerComponents.back()->applyTheme(
                    theme, "gamelist", element.first, ALL);
                addThemedChild(mGenericContainerComponents.back().get(), element.first);
            }
            else if (element.second.type == "rectangle" &&
                     !(element.second.has("visible") && !element.second.get<bool>("visible"))) {
                mRectangleComponents.push_back(std::make_unique<RectangleComponent>());
                mRectangleComponents.back()->setDefaultZIndex(35.0f);
                mRectangleComponents.back()->applyTheme(theme, "gamelist", element.first, ALL);
                addThemedChild(mRectangleComponents.back().get(), element.first);
            }
            else if (element.second.type == "screenshader" &&
                     !(element.second.has("visible") && !element.second.get<bool>("visible"))) {
                mScreenShaderComponents.push_back(std::make_unique<ScreenShaderComponent>());
                mScreenShaderComponents.back()->setDefaultZIndex(49.0f);
                mScreenShaderComponents.back()->applyTheme(theme, "gamelist", element.first, ALL);
                addThemedChild(mScreenShaderComponents.back().get(), element.first);
            }
            else if (element.second.type == "ninepatch" &&
                     !(element.second.has("visible") && !element.second.get<bool>("visible"))) {
                mNinePatchComponents.push_back(std::make_unique<NinePatchComponent>());
                mNinePatchComponents.back()->setDefaultZIndex(35.0f);
                mNinePatchComponents.back()->applyTheme(theme, "gamelist", element.first, ALL);
                addThemedChild(mNinePatchComponents.back().get(), element.first);
            }
            else if (element.second.type == "webimage" &&
                     !(element.second.has("visible") && !element.second.get<bool>("visible"))) {
                mWebImageComponents.push_back(std::make_unique<WebImageComponent>());
                mWebImageComponents.back()->setDefaultZIndex(30.0f);
                mWebImageComponents.back()->applyTheme(theme, "gamelist", element.first, ALL);
                addThemedChild(mWebImageComponents.back().get(), element.first);
            }
            if (element.second.type == "image" &&
                ...
                if (mImageComponents.back()->getThemeImageTypes().size() != 0)
                    ...
                    mImageComponents.back()->setScrollHide(true);
                addThemedChild(mImageComponents.back().get(), element.first);
            }
                ...
                if (element.second.has("path")) {
                    ...
                    addThemedChild(mStaticVideoComponents.back().get(), element.first);
                }
                else {
                    ...
                    addThemedChild(mVideoComponents.back().get(), element.first);
                }
            }
                ...
                if (extension == ".json") {
                    ...
                    addThemedChild(mLottieAnimComponents.back().get(), element.first);
                }
                    ...
                    addThemedChild(mGIFAnimComponents.back().get(), element.first);
                }
                else if (extension == ".") {
                    LOG(LogWarning)
                ...
                addThemedChild(mBadgeComponents.back().get(), element.first);
            }
                ...
                if (container) {
                    ...
                    if (mContainerTextComponents.back()->getThemeMetadata() != "")
                        ...
                        mContainerComponents.back()->setScrollHide(true);
                    addThemedChild(mContainerComponents.back().get(), element.first);
                }
                else {
                    ...
                    if (metadata != "" && metadata != "systemName" &&
                        ...
                        mTextComponents.back()->setScrollHide(true);
                    addThemedChild(mTextComponents.back().get(), element.first);
                }
            }
                ...
                addThemedChild(mDateTimeComponents.back().get(), element.first);
            }
                ...
                addThemedChild(mGamelistInfoComponents.back().get(), element.first);
            }
                ...
                addThemedChild(mRatingComponents.back().get(), element.first);
            }
                ...
                mControllerActivityComponents.emplace_back(
                    std::make_unique<ControllerActivityComponent>());
                mControllerActivityComponents.back()->setDefaultZIndex(45.0f);
                mControllerActivityComponents.back()->applyTheme(
                    theme, "gamelist", element.first, ALL);
                addThemedChild(mControllerActivityComponents.back().get(), element.first);
            }
        }
    }
...
bool GamelistView::input(InputConfig* config, Input input)
{
    ...
    return GamelistBase::input(config, input);
}
...
void GamelistView::updateView(const CursorState& state)
{
    ...
    if (file != nullptr)
        updateGameBindingVariables(file);
    ...
    const bool activateExtras {state == CursorState::CURSOR_STOPPED && file != nullptr};
    for (unsigned int i {0}; i < getChildCount(); ++i) {
        GuiComponent* child {getChild(i)};
        if (child == mPrimary)
            continue;
        ...
        triggerGamelistStoryboardRecursive(child, activateExtras);
    }
...
void GamelistView::setGameImage(FileData* file, GuiComponent* comp)
{
    ...
    for (auto& imageType : comp->getThemeImageTypes()) {
        ...
        } else if (imageType == "marquee" || imageType == "logo") {
            // 1. Si el theme define un logo/marquee explícito, úsalo
            path = file->getMarqueePath();
            if (!path.empty()) {
                comp->setImage(path);
                return;
            }
            // 2. Si no, intenta buscar logo del juego por scraping/metadata
            // (Batocera: busca logo automáticamente aunque no esté en el theme)
            // Aquí podrías agregar lógica extra si hay métodos alternativos de logo
    ...
    // Fallback Batocera: si es carousel y no hay logo/marquee, intenta buscar logo automáticamente
    if ((comp->getThemeImageTypes().size() == 1 &&
         (comp->getThemeImageTypes()[0] == "marquee" || comp->getThemeImageTypes()[0] == "logo")) &&
        path.empty()) {
        // Intenta buscar logo del juego (por ejemplo, scraping, metadata, etc.)
        path = file->getMarqueePath();
        if (!path.empty()) {
            comp->setImage(path);
            return;
    ...
    // Si sigue vacío, usa el default
    if (path.empty() && (comp->getThemeImageTypes().size() > 0 || !comp->getDefaultImage().empty())) {
        comp->setImage("");
        return;
    }
