...
namespace {
void triggerStoryboardRecursive(GuiComponent* component, bool activate)
{
    ...
    if (component->isStoryBoardRunning("activate") ||
        component->isStoryBoardRunning("deactivate")) {
        component->deselectStoryboard(false);
    }

    if (component->selectStoryboard(activate ? "activate" : "deactivate")) {
        component->startStoryboard();
        component->update(0);
        handled = true;
    }
    else if (activate && component->selectStoryboard("open")) {
        component->startStoryboard();
        component->update(0);
        handled = true;
    }
    else if (activate && (component->selectStoryboard("show") || component->selectStoryboard())) {
        component->startStoryboard();
        component->update(0);
        handled = true;
    }
    else if (!activate && component->selectStoryboard("close")) {
        component->startStoryboard();
        component->update(0);
        handled = true;
    }
    else if (!activate && component->selectStoryboard("hide")) {
        component->startStoryboard();
        component->update(0);
        handled = true;
    }
    ...
    for (unsigned int i {0}; i < component->getChildCount(); ++i)
        triggerStoryboardRecursive(component->getChild(i), activate);
}

void triggerSystemChildren(std::vector<GuiComponent*>& children, bool activate)
{
    for (GuiComponent* child : children)
        triggerStoryboardRecursive(child, activate);
}

void updateSystemChildren(std::vector<GuiComponent*>& children, int deltaTime)
{
    for (GuiComponent* child : children) {
        if (dynamic_cast<VideoFFmpegComponent*>(child) == nullptr)
            child->update(deltaTime);
    }
}

const ThemeData::ThemeSubset* findLogoSubset(const ThemeData::Theme& theme)
{
    for (const auto& subset : theme.capabilities.subsets) {
        if (Utils::String::toLower(subset.name) == "logos arcade power")
            return &subset;
    }

    for (const auto& subset : theme.capabilities.subsets) {
        const std::string normalizedName {Utils::String::toLower(subset.name)};

        if (normalizedName.find("logo") == std::string::npos)
            continue;

        if (normalizedName.find("menu") != std::string::npos ||
            normalizedName.find("systemview") != std::string::npos ||
            normalizedName.find("colorset") != std::string::npos) {
            continue;
        }
...
std::vector<std::string> getLogoSearchFolders(const std::string& selectedLogoPack)
{
    const std::string normalizedSelection {Utils::String::toLower(selectedLogoPack)};

    if (normalizedSelection.find("texto") != std::string::npos ||
        normalizedSelection.find("text") != std::string::npos) {
        return {};
    }

    if (normalizedSelection.find("arcade") != std::string::npos) {
        return {"/_arte/imgs/arcade/", "/_arte/imgs/basicos/", "/_arte/logos/",
                "/_arte/consoles/"};
    }

    if (normalizedSelection.find("basic") != std::string::npos ||
        normalizedSelection.find("basico") != std::string::npos ||
        normalizedSelection.find("basicos") != std::string::npos) {
        return {"/_arte/imgs/basicos/", "/_arte/imgs/arcade/", "/_arte/logos/",
                "/_arte/consoles/"};
    }
...
SystemView::SystemView()
    ...
    , mSystemInfoDelayMs {0}
    , mSystemInfoCountOnly {false}
    , mPendingGameCountUpdate {false}
    , mPendingGameCountDelayMs {0}
{
...
void SystemView::onShow()
{
    finishAnimation(0);
    stopViewVideos();
    if (mStaticBackgroundVideo != nullptr) {
        mStaticBackgroundVideo->setStaticVideo();
        mStaticBackgroundVideo->startVideoPlayer();
    }
    mFadeOpacity = 0.0f;
    mTransitionAnim = false;
    mPreviousSelectEvent = -1;
    ...
    if (mPrimary != nullptr && mPrimary->getCursor() + 1 <= static_cast<int>(mSystemElements.size())) {
        triggerSystemChildren(mSystemElements[mPrimary->getCursor()].children, true);
    }
}
...
void SystemView::onHide()
{
    ...
    triggerSystemChildren(mSystemElements[mPrimary->getCursor()].children, false);

    for (auto& video : mSystemElements[mPrimary->getCursor()].videoComponents)
        video->stopVideoPlayer(false);

    if (mStaticBackgroundVideo != nullptr)
        mStaticBackgroundVideo->stopVideoPlayer(false);
...
bool SystemView::input(InputConfig* config, Input input)
{
    ...
    if (!mSystemElements.empty() && mPrimary != nullptr) {
        for (auto& controllerActivity :
             mSystemElements[mPrimary->getCursor()].controllerActivityComponents) {
            controllerActivity->input(config, input);
        }
    }
...
{
    if (mPendingGameCountUpdate) {
        mPendingGameCountDelayMs -= deltaTime;
        if (mPendingGameCountDelayMs <= 0) {
            mPendingGameCountUpdate = false;
            updateGameCount();
        }
    }
    ...
    for (auto& systemElement : mSystemElements)
        updateSystemChildren(systemElement.children, deltaTime);

    if (mStaticBackgroundVideo != nullptr && !isScrolling())
        mStaticBackgroundVideo->update(deltaTime);
...
void SystemView::render(const glm::mat4& parentTrans)
{
    ...
    if (mStaticBackgroundVideo != nullptr) {
        GuiComponent* staticBackground {mStaticBackgroundVideo.get()};
        staticBackground->render(getTransform() * parentTrans);
    }
...
void SystemView::queueGameCountUpdate()
{
    if (mSystemInfoDelayMs <= 0) {
        mPendingGameCountUpdate = false;
        mPendingGameCountDelayMs = 0;
        updateGameCount();
        return;
    ...
    mPendingGameCountUpdate = true;
    mPendingGameCountDelayMs = mSystemInfoDelayMs;
}
...
void SystemView::onCursorChanged(const CursorState& state)
{
    ...
    const bool cursorChanged {cursor != mLastCursor};
    ...
    if (cursorChanged && mLastCursor >= 0 &&
        mLastCursor < static_cast<int>(mSystemElements.size())) {
        triggerSystemChildren(mSystemElements[mLastCursor].children, false);
    ...
    if (cursorChanged)
        triggerSystemChildren(mSystemElements[cursor].children, true);
...
void SystemView::populate()
{
    ...
    const ThemeData::ThemeSubset* logoSubset {findLogoSubset(selectedTheme->second)};
    const std::string logoSubsetSettingKey {
        logoSubset != nullptr ? "ThemeSubset_" + logoSubset->name : std::string {}};

    auto findSystemCarouselLogoPath = [logoSubsetSettingKey](SystemData* system) -> std::string {
        if (system == nullptr)
            return {};

        const std::string themePath {system->getThemePath()};
        if (themePath.empty())
            return {};

        const std::string themeRoot {Utils::FileSystem::getParent(themePath)};
        const std::string themeFolder {system->getThemeFolder()};
        const std::string systemName {system->getName()};
        // Siempre usar el subset/logo-pack seleccionado por el usuario si existe
        std::string selectedLogoPack;
        if (!logoSubsetSettingKey.empty() && Settings::getInstance()->hasString(logoSubsetSettingKey))
            selectedLogoPack = Settings::getInstance()->getString(logoSubsetSettingKey);
        else
            selectedLogoPack = "";

        std::vector<std::string> keys;
        keys.emplace_back(themeFolder);
        if (systemName != themeFolder)
            keys.emplace_back(systemName);

        // Si no hay subset seleccionado, usar el orden por defecto ArcadePower
        const std::vector<std::string> searchFolders = getLogoSearchFolders(selectedLogoPack);
        if (searchFolders.empty()) {
            // Si el subset es texto, no forzar imagen
            return {};
        }
        ...
        for (const std::string& key : keys) {
            if (key.empty())
                continue;
            for (const std::string& folder : searchFolders) {
                const std::vector<std::string> candidates {
                    themeRoot + folder + key + ".svg",
                    themeRoot + folder + key + ".png"
                };
                for (const std::string& candidate : candidates) {
                    if (Utils::FileSystem::exists(candidate) &&
                        !Utils::FileSystem::isDirectory(candidate)) {
                        return candidate;
                    }
                }
            }
    ...
    for (auto it : SystemData::sSystemVector) {
        ...
        if (theme->hasView("system")) {
            ...
            std::unordered_map<std::string, StackPanelComponent*> stackPanels;
            auto getLogicalName = [](const std::string& elementKey) {
                const size_t splitPos {elementKey.find('_')};
                return splitPos == std::string::npos ? elementKey : elementKey.substr(splitPos + 1);
            };
            auto addThemedChild = [&](GuiComponent* component, const std::string& elementKey) {
                const std::string logicalName {getLogicalName(elementKey)};
                const size_t splitPos {logicalName.rfind('.')};
                if (splitPos != std::string::npos) {
                    const std::string parentName {logicalName.substr(0, splitPos)};
                    auto parentIt = stackPanels.find(parentName);
                    if (parentIt != stackPanels.end()) {
                        parentIt->second->addChild(component);
                        return;
                    }
                }

                elements.children.emplace_back(component);
            };
            ...
            for (auto& element : theme->getViewElements("system").elements) {
                ...
                const bool gridLike {element.second.type == "grid" ||
                                     element.second.type == "imagegrid"};

                if (element.second.type == "carousel" || gridLike ||
                    ...
                    if (gridLike &&
                        ...
                        LOG(LogWarning) << "SystemView::populate(): Multiple primary components "
                                        << "defined, skipping grid/imagegrid configuration entry";
                        continue;
                    }
                    ...
                    if (element.second.type == "carousel" && mCarousel == nullptr) {
                        ...
                        mPrimaryType = PrimaryType::CAROUSEL;
                        if (element.second.has("systemInfoDelay")) {
                            mSystemInfoDelayMs = static_cast<int>(glm::clamp(
                                element.second.get<float>("systemInfoDelay"), 0.0f, 10000.0f));
                        }
                        if (element.second.has("systemInfoCountOnly"))
                            mSystemInfoCountOnly = element.second.get<bool>("systemInfoCountOnly");
                    }
                    else if (gridLike && mGrid == nullptr) {
                ...
                else if (element.second.type == "stackpanel" &&
                         !(element.second.has("visible") &&
                           !element.second.get<bool>("visible"))) {
                    elements.stackPanelComponents.emplace_back(
                        std::make_unique<StackPanelComponent>());
                    elements.stackPanelComponents.back()->setDefaultZIndex(35.0f);
                    elements.stackPanelComponents.back()->applyTheme(
                        theme, "system", element.first, ThemeFlags::ALL);

                    const std::string logicalName {getLogicalName(element.first)};
                    stackPanels[logicalName] = elements.stackPanelComponents.back().get();
                    addThemedChild(elements.stackPanelComponents.back().get(), element.first);
                }
                else if (element.second.type == "container" &&
                         !(element.second.has("visible") &&
                           !element.second.get<bool>("visible"))) {
                    elements.genericContainerComponents.emplace_back(
                        std::make_unique<ContainerComponent>());
                    elements.genericContainerComponents.back()->setDefaultZIndex(35.0f);
                    elements.genericContainerComponents.back()->applyTheme(
                        theme, "system", element.first, ThemeFlags::ALL);
                    addThemedChild(elements.genericContainerComponents.back().get(), element.first);
                }
                else if (element.second.type == "rectangle" &&
                         !(element.second.has("visible") &&
                           !element.second.get<bool>("visible"))) {
                    elements.rectangleComponents.emplace_back(
                        std::make_unique<RectangleComponent>());
                    elements.rectangleComponents.back()->setDefaultZIndex(35.0f);
                    elements.rectangleComponents.back()->applyTheme(
                        theme, "system", element.first, ThemeFlags::ALL);
                    addThemedChild(elements.rectangleComponents.back().get(), element.first);
                }
                else if (element.second.type == "screenshader" &&
                         !(element.second.has("visible") &&
                           !element.second.get<bool>("visible"))) {
                    elements.screenShaderComponents.emplace_back(
                        std::make_unique<ScreenShaderComponent>());
                    elements.screenShaderComponents.back()->setDefaultZIndex(49.0f);
                    elements.screenShaderComponents.back()->applyTheme(
                        theme, "system", element.first, ThemeFlags::ALL);
                    addThemedChild(elements.screenShaderComponents.back().get(), element.first);
                }
                else if (element.second.type == "ninepatch" &&
                         !(element.second.has("visible") &&
                           !element.second.get<bool>("visible"))) {
                    elements.ninePatchComponents.emplace_back(std::make_unique<NinePatchComponent>());
                    elements.ninePatchComponents.back()->setDefaultZIndex(35.0f);
                    elements.ninePatchComponents.back()->applyTheme(
                        theme, "system", element.first, ThemeFlags::ALL);
                    addThemedChild(elements.ninePatchComponents.back().get(), element.first);
                }
                else if (element.second.type == "webimage" &&
                         !(element.second.has("visible") &&
                           !element.second.get<bool>("visible"))) {
                    elements.webImageComponents.emplace_back(std::make_unique<WebImageComponent>());
                    elements.webImageComponents.back()->setDefaultZIndex(30.0f);
                    elements.webImageComponents.back()->applyTheme(
                        theme, "system", element.first, ThemeFlags::ALL);
                    addThemedChild(elements.webImageComponents.back().get(), element.first);
                }
                    ...
                    elements.imageComponents.back()->applyTheme(theme, "system", element.first,
                                                                ThemeFlags::ALL);
                    addThemedChild(elements.imageComponents.back().get(), element.first);
                }
                else if (element.second.type == "video" &&
                         !(element.second.has("visible") && !element.second.get<bool>("visible"))) {
                    const std::string normalizedElementName {
                        Utils::String::toLower(element.first)};

                    if (normalizedElementName == "video_staticbackgroundvideo") {
                        if (mStaticBackgroundVideo == nullptr) {
                            mStaticBackgroundVideo = std::make_unique<VideoFFmpegComponent>();
                            mStaticBackgroundVideo->setDefaultZIndex(30.0f);
                            mStaticBackgroundVideo->setStaticVideo();
                            mStaticBackgroundVideo->applyTheme(theme, "system", element.first,
                                                               ThemeFlags::ALL);
                        }
                        continue;
                    }
                    ...
                    elements.videoComponents.back()->applyTheme(theme, "system", element.first,
                                                                ThemeFlags::ALL);
                    addThemedChild(elements.videoComponents.back().get(), element.first);
                }
                    ...
                    if (extension == ".json") {
                        ...
                        elements.lottieAnimComponents.back()->applyTheme(
                            theme, "system", element.first, ThemeFlags::ALL);
                        addThemedChild(elements.lottieAnimComponents.back().get(), element.first);
                    }
                        ...
                        elements.GIFAnimComponents.back()->applyTheme(
                            theme, "system", element.first, ThemeFlags::ALL);
                        addThemedChild(elements.GIFAnimComponents.back().get(), element.first);
                    }
                    else if (extension == ".") {
                    ...
                    if (element.second.has("systemdata") &&
                        ...
                        if (element.second.has("systemdata")) {
                            ...
                            elements.gameCountComponents.back()->applyTheme(
                                theme, "system", element.first, ThemeFlags::ALL);
                            addThemedChild(elements.gameCountComponents.back().get(),
                                           element.first);
                        }
                    }
                    else {
                        if (container) {
                            ...
                            elements.containerTextComponents.back()->applyTheme(
                                ...
                                ThemeFlags::ALL ^ ThemeFlags::POSITION ^ ThemeFlags::ORIGIN ^
                                    ...
                                    ThemeFlags::ROTATION);
                            addThemedChild(elements.containerComponents.back().get(),
                                           element.first);
                        }
                        else {
                            ...
                            elements.textComponents.back()->applyTheme(
                                theme, "system", element.first, ThemeFlags::ALL);
                            addThemedChild(elements.textComponents.back().get(), element.first);
                        }
                    }
                }
                    ...
                    addThemedChild(elements.dateTimeComponents.back().get(), element.first);
                }
                    ...
                    addThemedChild(elements.ratingComponents.back().get(), element.first);
                }
                    ...
                    elements.clockComponents.back()->applyTheme(theme, "system", element.first,
                         ...
                         element.second.type == "batteryIndicator") {
                ...
                else if (element.second.type == "controllerActivity") {
                    elements.controllerActivityComponents.emplace_back(
                        std::make_unique<ControllerActivityComponent>());
                    elements.controllerActivityComponents.back()->setDefaultZIndex(45.0f);
                    elements.controllerActivityComponents.back()->applyTheme(
                        theme, "system", element.first, ThemeFlags::ALL);
                    addThemedChild(elements.controllerActivityComponents.back().get(),
                                   element.first);
                }
            }
        }
        ...
        if (mCarousel != nullptr) {
            ...
            entry.object = it;
            if (imagePath.empty())
                imagePath = findSystemCarouselLogoPath(it);
