...
void ViewController::setMenuColors()
{
    ...
    mMenuImagePathSwitchOn = ":/graphics/on.svg";
    mMenuImagePathSwitchOff = ":/graphics/off.svg";
    mMenuImagePathSliderKnob = ":/graphics/slider_knob.svg";
    mMenuImagePathButton = ":/graphics/button.svg";
    mMenuImagePathButtonFilled = ":/graphics/button_filled.svg";
    mMenuImagePathTextInput = ":/graphics/textinput.svg";
    mMenuImagePathTextInputFocused = ":/graphics/textinput_focused.svg";
    mMenuImagePathBackground.clear();
    mMenuBackgroundCornerSize = glm::vec2 {30.0f, 30.0f};
    mMenuFontPathTitle.clear();
    mMenuFontSizeTitle = 0.0f;
    mMenuFontPathPrimary.clear();
    mMenuFontSizePrimary = 0.0f;
    mMenuFontPathSecondary.clear();
    mMenuFontSizeSecondary = 0.0f;
    mMenuColorFooter = mMenuColorPopupText;
    mMenuFontPathFooter.clear();
    mMenuFontSizeFooter = 0.0f;

    mMenuColorSelectorEnd = mMenuColorSelector;
    mMenuColorSelectorGradientHorizontal = true;
    mMenuSelectorOffsetY = 0.0f;
    mMenuLineSpacing = 1.0f;
    mMenuColorGroupBackground = 0x00000000;

    // Optional theme overrides for menu widgets and Batocera aliases.
    if (!SystemData::sSystemVector.empty()) {
        auto getMenuElement = [](const std::shared_ptr<ThemeData>& theme,
                                 const std::vector<std::string>& names,
                                 const std::string& expectedType)
            -> const ThemeData::ThemeElement* {
            if (!theme)
                return nullptr;

            for (const auto& name : names) {
                const ThemeData::ThemeElement* elem {
                    theme->getElement("menu", name, expectedType)};
                if (elem != nullptr)
                    return elem;
            }
        ...
        if (mState.viewing == ViewMode::GAMELIST || mState.viewing == ViewMode::SYSTEM_SELECT)
            themeSystem = mState.getSystem();
        ...
        const ThemeData::ThemeElement* titleElem {getMenuElement(theme, {"menutitle"},
                                                                 "menuText")};
        const ThemeData::ThemeElement* footerElem {getMenuElement(theme, {"menufooter"},
                                                                  "menuText")};
        const ThemeData::ThemeElement* textElem {
            getMenuElement(theme, {"menuText", "menutext"}, "menuText")};        const ThemeData::ThemeElement* textEditElem {
            getMenuElement(theme, {"menuTextEdit", "menutextedit"}, "menuTextEdit")};
        const ThemeData::ThemeElement* textSmallElem {
            getMenuElement(theme, {"menuTextSmall", "menutextsmall"}, "menuTextSmall")};
        const ThemeData::ThemeElement* backgroundElem {
            getMenuElement(theme, {"menuBackground", "menubg"}, "menuBackground")};
        const ThemeData::ThemeElement* switchElem {
            getMenuElement(theme, {"menuSwitch", "menuswitch"}, "menuSwitch")};
        const ThemeData::ThemeElement* sliderElem {
            getMenuElement(theme, {"menuSlider", "menuslider"}, "menuSlider")};
        const ThemeData::ThemeElement* buttonElem {
            getMenuElement(theme, {"menuButton", "menubutton"}, "menuButton")};
        const ThemeData::ThemeElement* groupElem {
            getMenuElement(theme, {"menuGroup", "menugroup"}, "menuGroup")};
        const ThemeData::ThemeElement* gridElem {
            getMenuElement(theme, {"menuGrid", "menugrid"}, "menuGrid")};

        if (titleElem != nullptr) {
            if (titleElem->has("color"))
                mMenuColorTitle = titleElem->get<unsigned int>("color");
            if (titleElem->has("fontPath"))
                mMenuFontPathTitle = titleElem->get<std::string>("fontPath");
            if (titleElem->has("fontSize"))
                mMenuFontSizeTitle = glm::clamp(screenSize * titleElem->get<float>("fontSize"),
                                                screenSize * 0.001f, screenSize * 1.5f);
        }

        if (footerElem != nullptr) {
            if (footerElem->has("color"))
                mMenuColorFooter = footerElem->get<unsigned int>("color");
            if (footerElem->has("fontPath"))
                mMenuFontPathFooter = footerElem->get<std::string>("fontPath");
            if (footerElem->has("fontSize"))
                mMenuFontSizeFooter = glm::clamp(screenSize * footerElem->get<float>("fontSize"),
                                                 screenSize * 0.001f, screenSize * 1.5f);
        }

        // menuText: main list item colors and separator/selector.
        if (textElem != nullptr) {
            if (textElem->has("color"))
                mMenuColorPrimary = textElem->get<unsigned int>("color");
            if (textElem->has("fontPath"))
                mMenuFontPathPrimary = textElem->get<std::string>("fontPath");
            if (textElem->has("fontSize"))
                mMenuFontSizePrimary = glm::clamp(screenSize * textElem->get<float>("fontSize"),
                                                  screenSize * 0.001f, screenSize * 1.5f);
            if (textElem->has("selectedColor"))
                mMenuColorButtonTextFocused = textElem->get<unsigned int>("selectedColor");
            if (textElem->has("selectorColor"))
                mMenuColorSelector = textElem->get<unsigned int>("selectorColor");
            mMenuColorSelectorEnd = mMenuColorSelector;
            if (textElem->has("selectorColorEnd"))
                mMenuColorSelectorEnd = textElem->get<unsigned int>("selectorColorEnd");
            if (textElem->has("selectorGradientType")) {
                const std::string& gradientType {textElem->get<std::string>("selectorGradientType")};
                if (gradientType == "horizontal") {
                    mMenuColorSelectorGradientHorizontal = true;
                }
                else if (gradientType == "vertical") {
                    mMenuColorSelectorGradientHorizontal = false;
                }
                else {
                    LOG(LogWarning) << "ViewController::setMenuColors(): Invalid theme configuration "
                                       "value \""
                                    << gradientType << "\" for property "
                                       "\"selectorGradientType\" for element \""
                                    << "menuText\"";
                    mMenuColorSelectorGradientHorizontal = true;
                }
            }
            if (textElem->has("selectorOffsetY"))
                mMenuSelectorOffsetY = glm::clamp(textElem->get<float>("selectorOffsetY"),
                                                  -1.0f, 1.0f);
            if (textElem->has("separatorColor"))
                mMenuColorSeparators = textElem->get<unsigned int>("separatorColor");
        }

        // menuTextEdit: keyboard / text input widget colors.
        if (textEditElem != nullptr) {
            if (textEditElem->has("color"))
                mMenuColorKeyboardText = textEditElem->get<unsigned int>("color");
            if (textEditElem->has("inactive"))
                mMenuColorTextInputFrameUnfocused = textEditElem->get<unsigned int>("inactive");
            if (textEditElem->has("active"))
                mMenuColorTextInputFrameFocused = textEditElem->get<unsigned int>("active");
            if (textEditElem->has("inactivePath"))
                mMenuImagePathTextInput = textEditElem->get<std::string>("inactivePath");
            if (textEditElem->has("activePath"))
                mMenuImagePathTextInputFocused = textEditElem->get<std::string>("activePath");
        }

        // menuTextSmall: small / secondary text (help prompts, sub-labels).
        if (textSmallElem != nullptr) {
            if (textSmallElem->has("color"))
                mMenuColorSecondary = textSmallElem->get<unsigned int>("color");
            if (textSmallElem->has("fontPath"))
                mMenuFontPathSecondary = textSmallElem->get<std::string>("fontPath");
            if (textSmallElem->has("fontSize"))
                mMenuFontSizeSecondary =
                    glm::clamp(screenSize * textSmallElem->get<float>("fontSize"),
                               screenSize * 0.001f, screenSize * 1.5f);
        }

        // menuBackground: base menu frame tone.
        if (backgroundElem != nullptr) {
            if (backgroundElem->has("centerColor")) {
                const unsigned int centerColor {backgroundElem->get<unsigned int>("centerColor")};
                mMenuColorFrame = centerColor;
                mMenuColorFrameLaunchScreen = centerColor;
                mMenuColorFrameBusyComponent = centerColor;
            }
            if (backgroundElem->has("color")) {
                const unsigned int bgColor {backgroundElem->get<unsigned int>("color")};
                mMenuColorFrame = bgColor;
                mMenuColorFrameLaunchScreen = bgColor;
                mMenuColorFrameBusyComponent = bgColor;
            }
            if (backgroundElem->has("path"))
                mMenuImagePathBackground = backgroundElem->get<std::string>("path");
            if (backgroundElem->has("cornerSize"))
                mMenuBackgroundCornerSize = backgroundElem->get<glm::vec2>("cornerSize");
            if (backgroundElem->has("scrollbarColor"))
                mMenuColorScrollIndicators = backgroundElem->get<unsigned int>("scrollbarColor");
        }

        // menuSwitch: toggle widget colors.
        if (switchElem != nullptr) {
            if (switchElem->has("color"))
                mMenuColorPrimary = switchElem->get<unsigned int>("color");
            if (switchElem->has("inactive"))
                mMenuColorSecondary = switchElem->get<unsigned int>("inactive");
            if (switchElem->has("active"))
                mMenuColorSelector = switchElem->get<unsigned int>("active");
            if (switchElem->has("pathOn"))
                mMenuImagePathSwitchOn = switchElem->get<std::string>("pathOn");
            if (switchElem->has("pathOff"))
                mMenuImagePathSwitchOff = switchElem->get<std::string>("pathOff");
        }

        // menuSlider: slider widget colors.
        if (sliderElem != nullptr) {
            if (sliderElem->has("color"))
                mMenuColorPrimary = sliderElem->get<unsigned int>("color");
            if (sliderElem->has("inactive"))
                mMenuColorSliderKnobDisabled = sliderElem->get<unsigned int>("inactive");
            if (sliderElem->has("active"))
                mMenuColorSelector = sliderElem->get<unsigned int>("active");
            if (sliderElem->has("path"))
                mMenuImagePathSliderKnob = sliderElem->get<std::string>("path");
        }

        // menuButton: button widget colors (focused / unfocused backgrounds and text).
        if (buttonElem != nullptr) {
            if (buttonElem->has("color"))
                mMenuColorButtonFlatUnfocused = buttonElem->get<unsigned int>("color");
            if (buttonElem->has("colorFocused"))
                mMenuColorButtonFocused = buttonElem->get<unsigned int>("colorFocused");
            if (buttonElem->has("textColor"))
                mMenuColorButtonTextUnfocused = buttonElem->get<unsigned int>("textColor");
            if (buttonElem->has("textColorFocused"))
                mMenuColorButtonTextFocused = buttonElem->get<unsigned int>("textColorFocused");
            if (buttonElem->has("path"))
                mMenuImagePathButton = buttonElem->get<std::string>("path");
            if (buttonElem->has("filledPath"))
                mMenuImagePathButtonFilled = buttonElem->get<std::string>("filledPath");
        }

        // menuGroup: section group header text and separators.
        if (groupElem != nullptr) {
            if (groupElem->has("color"))
                mMenuColorTertiary = groupElem->get<unsigned int>("color");
            if (groupElem->has("separatorColor"))
                mMenuColorSeparators = groupElem->get<unsigned int>("separatorColor");
            if (groupElem->has("backgroundColor"))
                mMenuColorGroupBackground = groupElem->get<unsigned int>("backgroundColor");
            if (groupElem->has("lineSpacing"))
                mMenuLineSpacing = glm::clamp(groupElem->get<float>("lineSpacing"), 0.8f, 1.5f);
        }

        // menuGrid: Batocera alias for separator styling.
        if (gridElem != nullptr) {
            if (gridElem->has("separatorColor"))
                mMenuColorSeparators = gridElem->get<unsigned int>("separatorColor");
        }
    }
}
