...
bool SystemData::isCheevosSupported() const
{
    if (isCollection() || hasPlatformId(PlatformIds::PLATFORM_IGNORE) || !isGameSystem())
        return false;

    return !mEnvData->mPlatformIds.empty();
}

bool SystemData::getShowCheevosIcon() const
{
    return isCheevosSupported() && Settings::getInstance()->getBool("ShowCheevosIcon");
}
...
void SystemData::loadTheme(ThemeTriggers::TriggerType trigger)
{
    ...
    try {
        ...
        sysData.insert(std::pair<std::string, std::string>(
            "system.collection", isCollection() ? "true" : "false"));
        if (isCollection() && isCustomCollection()) {
            sysData.insert(
