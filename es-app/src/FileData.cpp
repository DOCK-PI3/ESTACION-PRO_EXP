...
{
    LOG(LogInfo) << "FileData::getMarqueePath(): Resolving marquee for game \""
                 << getDisplayName() << "\" in system \"" << mSystemName << "\"";

    // Batocera/Retrobat compatibility: read metadata-backed logo paths first.
    // This allows themes using gamecarousel imageSource=marquee to resolve logos from
    // gamelist.xml entries (<marquee>, <wheel>, <logo>) in addition to media directories.
    for (const std::string& key : {std::string {"marquee"}, std::string {"wheel"},
                                   std::string {"logo"}}) {
        const std::string metadataPath {metadata.get(key)};
        if (!metadataPath.empty() && Utils::FileSystem::exists(metadataPath)) {
            LOG(LogInfo) << "FileData::getMarqueePath(): Found metadata key \"" << key
                         << "\" -> \"" << metadataPath << "\"";
            return metadataPath;
        }

        LOG(LogInfo) << "FileData::getMarqueePath(): Metadata key \"" << key
                     << "\" " << (metadataPath.empty() ? "is empty" :
                                     "points to missing file \"" + metadataPath + "\"");
    }

    // Return path to the marquee image from media directories.
    std::string marqueePath {getMediafilePath("marquees")};
    if (!marqueePath.empty()) {
        LOG(LogInfo) << "FileData::getMarqueePath(): Found in media subdirectory \"marquees\""
                     << " -> \"" << marqueePath << "\"";
        return marqueePath;
    }

    LOG(LogInfo) << "FileData::getMarqueePath(): Not found in media subdirectory \"marquees\"";

    // Compatibility aliases used by external media sets.
    for (const std::string& subdirectory : {std::string {"wheels"}, std::string {"wheel"},
                                            std::string {"logos"}, std::string {"logo"}}) {
        marqueePath = getMediafilePath(subdirectory);
        if (!marqueePath.empty()) {
            LOG(LogInfo) << "FileData::getMarqueePath(): Found in media subdirectory \""
                         << subdirectory << "\" -> \"" << marqueePath << "\"";
            return marqueePath;
        }

        LOG(LogInfo) << "FileData::getMarqueePath(): Not found in media subdirectory \""
                     << subdirectory << "\"";
    }

    LOG(LogInfo) << "FileData::getMarqueePath(): No marquee found for game \""
                 << getDisplayName() << "\"";
...
int FileData::getCheevosGameId() const
{
    const FileData* sourceFile {mSourceFileData != nullptr ? mSourceFileData : this};
    return sourceFile->metadata.getInt("cheevos_id");
}

const std::string& FileData::getCheevosHash() const
{
    const FileData* sourceFile {mSourceFileData != nullptr ? mSourceFileData : this};
    return sourceFile->metadata.get("cheevos_hash");
}

bool FileData::hasCheevos() const
{
    return getCheevosGameId() > 0;
}

bool FileData::checkCheevosHash(const std::string& hash) const
{
    if (hash.empty())
        return false;

    return Utils::String::toLower(getCheevosHash()) == Utils::String::toLower(hash);
}
